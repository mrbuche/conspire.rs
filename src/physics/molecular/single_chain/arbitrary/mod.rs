...
impl ArbitraryDiscrete {
    fn frame_propagated_link_vectors(
        ...
        let mut b_previous: Option<Vector<Current>> = None;
...
fn random_bend_cos_theta<U>(potential: &U, temperature: Quantity<Temperature>) -> Scalar
where
    U: AngularPotential,
{
    loop {
        ...
        let weight = (-potential.energy(theta) / (BOLTZMANN_CONSTANT * temperature))
            .value()
            .exp();
...
fn random_torsion_phi<U>(potential: &U, temperature: Quantity<Temperature>) -> Scalar
where
    U: AngularPotential,
{
    ...
    loop {
        let phi = TAU * random_uniform();
        let weight = (-potential.energy(phi) / (BOLTZMANN_CONSTANT * temperature))
            .value()
            .exp();
        if random_uniform() < weight {
            return phi;
        ...
        if nondimensional_force != 0.0 {
            ...
            return (0..self.number_of_links())
                .map(|_| {
                    ...
                    let (sin_phi, cos_phi) = phi.sin_cos();
                    let lambda =
                        random_nondimensional_link_length(&self.link_potential, temperature, 0.0);
                    <Vector<Current>>::from([
