    ...
    V: Copy,
{
    ...
    fn pair_level(&mut self, coarse: usize, fine: usize) -> Result<bool, &'static str> {
        ...
        let coarse_nodes: Vec<(usize, [i32; D], bool)> = (0..self.len())
            .filter(|&index| self.nodes[index].length.cells() == coarse)
            .map(|index| {
                let required = self.nodes[index].is_tree();
                let corner = self.nodes[index]
                    .corner
                    .map(|coordinate| (coordinate.cells() / coarse) as i32);
                (index, corner, required)
            })
            .collect();
    ...
    fn straddling_jump(&self, vertex: &[i32; D], coarse: usize) -> bool {
        ...
        let leaf_at = |point: [i64; D]| -> Option<usize> {
        ...
        (0..D).filter(|&along| vertex[along] % 2 == 0).any(|along| {
            ...
            (0..3usize.pow(others.len() as u32)).any(|line| {
                ...
                let halves: [Vec<(i64, usize)>; 2] = from_fn(|half| {
                    ...
                    for shifts in 0..1usize << others.len() {
                        ...
                        if let Some(leaf) = leaf_at(point) {
                            cells.push((self.nodes[leaf].length.cells() as i64, leaf))
                        }
                    }
