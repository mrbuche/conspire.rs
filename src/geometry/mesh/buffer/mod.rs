...
impl Mesh<3> {
    ...
    /// isn't one of them is never pulled onto it; `corners` are the hard
    /// points several creases meet at, where a crease-owned node pins exactly
    /// instead of sliding along its curve's tangent. Both empty for a target
    /// with no such topology.
    pub(crate) fn buffer_with<O: fit::Oracle>(
        mut self,
        oracle: &O,
        creases: &[(Vec<Coordinate<3>>, Vec<usize>)],
        corners: &[Coordinate<3>],
        fitting: Fitting,
        ...
        mesh.fit(&nodes, oracle, creases, corners)?;
        if let Fitting::Snap = fitting {
            ...
            mesh.fit(&(0..count).collect::<Vec<_>>(), oracle, creases, corners)?;
        }
        Ok(mesh)
    }
