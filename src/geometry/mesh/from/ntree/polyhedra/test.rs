use super::super::test::{octree, quadtree};
use crate::{
    geometry::{
...
fn point<const D: usize>(mesh: &Mesh<D>, node: usize) -> [f64; D] {
    from_fn(|axis| mesh.coordinates()[node][axis].value())
}
...
fn octree_weak_2_faces() {
    ...
    tree.subdivide(0).unwrap();
    tree.subdivide(1).unwrap();
    tree.subdivide(10).unwrap();
    tree.subdivide(18).unwrap();
    tree.equilibrate(Balancing::Weak(2), Pairing::None).unwrap();
    let mesh = Mesh::from(tree);
    conformal(&mesh);
