    ...
    Self: Cohesive,
{
    ...
    fn stiffness(
        ...
        let stiffness_u: Stiffness = (0..3)
            .map(|i| {
                (0..3)
                    .map(|j| {
                        let mut u = separation.clone();
                        u[j] += 0.5 * EPSILON;
                        finite_difference = self.traction(u.clone(), normal.clone()).unwrap()[i];
                        u[j] -= EPSILON;
                        finite_difference -= self.traction(u.clone(), normal.clone()).unwrap()[i];
            ...
            .map(|i| {
                (0..3)
                    .map(|j| {
                        ...
                        n[j] += 0.5 * EPSILON;
                        finite_difference =
                            self.traction(separation.clone(), n.clone()).unwrap()[i];
                        n[j] -= EPSILON;
                        finite_difference -=
                            self.traction(separation.clone(), n.clone()).unwrap()[i];
                        finite_difference / EPSILON
