...
- The Cauchy stress and free energy are integrals over the end-to-end vectors
  of the network, split into a one-dimensional radial kernel (generalized
  Gauss-Laguerre) and an integral over the unit sphere (a product rule for
  now). The single-chain response inverts $`\gamma = \mathcal{L}(\eta) + \eta/\varkappa`$
