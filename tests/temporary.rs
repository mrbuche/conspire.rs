...
mod cbm_implicit_solve_smoke {
    ...
    #[test]
    fn root_via_model_and_first_order_root() {
        let (connectivity, reference_coordinates) = two_tetrahedra_reference();
        let constitutive_model = AlmansiHamelEulerian {
            bulk_modulus: Stress::pascals(13.0),
            shear_modulus: Stress::pascals(3.0),
        ...
        (0..4).for_each(|node| {
            (0..3).for_each(|component| {
                matrix[row][3 * node + component] = 1.0;
                vector[row] = reference_coordinates[node][component].value();
                row += 1;
            })
        });
        matrix[row][3 * 4] = 1.0;
        vector[row] = reference_coordinates[4][0].value() + 0.1;
        ...
        .unwrap();
        // Node 4 should relax in y,z away from the diagonal reference value
        // as the stretched bond pulls it, not stay pinned at the reference.
        assert!((coordinates[4][1].value() - reference_coordinates[4][1].value()).abs() > 1e-6);
    }
}
